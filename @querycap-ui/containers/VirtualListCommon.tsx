...
export function getOptions(options$: Observable<IVirtualListOptions>): Observable<IVirtualListOptions> {
  return options$.pipe(tap(validateOptions), map(defaultOptions));
}
...
export function getScrollTop(el: HTMLElement): Observable<number> {
  const scrollEvent$ = fromEvent(el, "scroll").pipe(startWith({ target: { scrollTop: 0 } }));
...
export function userScrollToPosition(options$: Observable<IVirtualListOptions>): Observable<number> {
  return options$.pipe(map((option) => option.startIndex! * option.height));
}

export function getStickyTop<T>(data$: Observable<T[]>, options$: Observable<IVirtualListOptions>) {
  return data$.pipe(
    withLatestFrom(options$),
...
export function getScrollDirection(scrollTop$: Observable<number>): Observable<number> {
...
export function getIndicesInViewport<T>(
...
export function getDataSliceInView<T>(
  data$: Observable<T[]>,
  options$: Observable<IVirtualListOptions>,
...
export function getScrollHeight<T>(
  data$: Observable<T[]>,
  options$: Observable<IVirtualListOptions>,
): Observable<number> {
  return combineLatest([data$, options$]).pipe(map(([data, option]) => data.length * option.height));
}
